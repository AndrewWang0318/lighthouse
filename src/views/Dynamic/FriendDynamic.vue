<template>
  <div class="page-friend-dynamic" @click="contentClike">
    <!-- 用户背景展示区 -->
    <div class="user-background-area">
      <div class="user-info-content">
        <div class="user-name">{{ userInfo.user_nickname }}</div>
        <div class="user-avatar" :style="`background-image: url(${base_url + userInfo.user_avatar});`"></div>
      </div>
      <!-- 发动态按钮 -->
      <div class="btn-upload-dynamic" ref="btn_upload" @click="uploadDynamic">
...
function uploadDynamic() { // 跳转到动态发布
  ...
  _API.dynamicList(parmas).then((res) => {
    ...
    dynamicList.forEach(element => {
      ...
      dynamic_like.forEach(item => {
        if (userInfo.user_id == item.like_user.user_id) is_liked = true // 本人id出现过点赞列表中
        liked_user.push(item.like_user.user_nickname)
      })
      // 距离当前时间
      let dynamic_time = moment(element.createdAt).fromNow();
      // 评论的数据
      let comment_content = []
      // 回复的数据
      let reply_content = []
...
function jumpInteract(i) {// 点赞及评论显示
  ...
  let element = instance.refs.moreContent[i];
  if (dynamicData[i].lc_show) {
    anime({ targets: element, width: "3rem", duration: 300, });
  } else {
    anime({ targets: element, easing: "cubicBezier(.18,.07,.24,.99)", width: "0", duration: 150, });
  }
}
...
function sendComment() { // 发送评论
  ...
  let comment_from_user_id = userInfo.user_id;
...
function media_preview(v, i) { // 预览图大图展示
  let arr = []
  v.forEach(item => {
