  ...
  <div class="page-selection">
    ...
    <div class="data-table" @mousemove="handleMouseMove" @mouseup="handleMouseUp">
      ...
      <div class="table" @mousedown="handleMouseDown">
        <div v-for="(v, i) in data" :key="`data${i}`"
          :class="`table-cell ${isSelected(v.id) ? 'selected' : ''}`" ref="dataItems"
          :data-id="v.id" @contextmenu.prevent="delItem(v.id)">
          {{ v.color }}
        </div>
      </div>
    </div>
...
for (let i = 0; i < 12; i++) {
  for (let j = 0; j < 6; j++) {
    const id = Mock.Random.natural(8);
